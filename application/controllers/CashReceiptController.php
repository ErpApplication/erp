...
class CashReceiptController extends CI_Controller
{
...
public function addCashReceipt(){
		...
		$this->load->view('assests/sidebar',$data);
		$this->load->view('assests/mainContent',$data);
	    $this->load->view('cashreceipt/addCashreceipt_View',$data);
	    $this->load->view('assests/footer');
	}
...
public function cashReceiptListwebAPI(){
    ...
    $data = $this->db->query("SELECT cash_receipt_id_PK as id,
        voucher_no
        FROM `tbl_cashreceipt`as cashreceipt
         ");
    ...
    $data12 = $this->db->query("SELECT cash_receipt_id_PK as id,
        voucher_no
        FROM `tbl_cashreceipt`as cashreceipt");
...
public function insertCashReceipt(){
	extract($_POST);
 ...
 //    // $this->form_validation->set_rules('iteamMaster_Item_Name','','required|trim|regex_match[/^[a-zA-Z ]+$/]',array('required'=>"Please Enter Item Name",'regex_match' => 'Please Enter character and space in Item Name'));  
    ...
    $this->form_validation->set_rules('cash_Receipt_Voucher_No_Id','','required', array('required'=>'Please Enter Voucher Number'));
       
    $this->form_validation->set_error_delimiters('<li class="err">', '</li></br>');
     
    if($this->form_validation->run()){      
        $allData=array(
        	'voucher_no'=>$cash_Receipt_Voucher_No_Id,
            'date'=>$cash_Receipt_Date_Id,
            'created_date'=>date('Y-m-d H:i:s')
        );
        if($uid ==""){
          $res=$this->Webservice_general_model->insert('tbl_cashreceipt',$allData);
        }else{
          $setFilterData['cash_receipt_id_PK']=$uid;
          $res = $this->Webservice_general_model->update('tbl_cashreceipt',$setFilterData, $allData);
        }


        for($i=0;$i<count($cash_Receipt_Party_Balance_Id);$i++){
          $allBankReceiptDetailData=array(
               'cash_receipt_id_FK'=>$res,
               'pary_balance'=>$cash_Receipt_Party_Balance_Id[$i],
               'party_id_FK'=>$cash_Receipt_Party_Value_Id[$i],
               'amount'=>$cash_Receipt_Amount_Id[$i],
               'receipt_no'=>$cash_Receipt_Receipt_No_Id[$i],
               'narration'=>$cash_Receipt_Narration_Id[$i],
               'balance'=>$cash_Receipt_Bal_Id[$i],
               'cash'=>$cash_Receipt_Cash_Id[$i],
               'cash_balance'=>$cash_Receipt_Balance_Id[$i],
               'book_id_FK'=>$cash_Receipt_Book_Id[$i],
               'naration2'=>$cash_Receipt_Narration_Two_Id[$i],
               'created_date'=>date('Y-m-d H:i:s')
            );
        $this->Webservice_general_model->insert('tbl_cashreceiptdetail',$allBankReceiptDetailData);
        }
        if($res)
        {
          $data["status"] = 1;
